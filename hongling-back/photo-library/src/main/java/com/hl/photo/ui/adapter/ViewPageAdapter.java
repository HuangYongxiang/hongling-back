...
import com.hl.photo.support.util.WindowUtil;
...
public class ViewPageAdapter extends PagerAdapter {
    ...
    private List<PhotoInfo> imageInfo;
    private SparseArray<View> cacheView;
    private ViewGroup containerTemp;

    //    public ViewPageAdapter(Context context, List<PhotoInfo> images) {
...
//        return super.getItemPosition(object);
    ...
    public Object instantiateItem(final ViewGroup container, int position) {
        ...
        if (view == null) {
            view = LayoutInflater.from(context).inflate(R.layout.photo_vp_item_image, container, false);
            view.setTag(position);
            ImageView image = (ImageView) view.findViewById(R.id.image);
            PhotoViewAttacher photoViewAttacher = new PhotoViewAttacher(image);
            String id = images.get(position);
            Glide.with(context).load(Constants.DOWNLOADPICTURE+"?id=" + id).asBitmap().into(new MyTarget(photoViewAttacher));
...
//                }
//            });
//            cacheView.put(position,view);//缓存导致无法刷新页面
        }
