package com.hl.core.lib.view.indexBar.widget;
...
public class IndexBar extends View {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.core_index_bar, defStyleAttr, 0);
        textSize = (int) typedArray.getDimension(R.styleable.core_index_bar_core_index_bar_text_size,textSize);
        mPressedBackground =  typedArray.getColor(R.styleable.core_index_bar_core_index_bar_press_background,mPressedBackground);
        typedArray.recycle();
        ...
        mPaint.setColor(ContextCompat.getColor(context,R.color.core_index_bar_text));
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        // 软键盘弹出时候，如果页面改变高度大于100,切不是横竖屏切换
        if (oldh-h > 100 && (oldh != w)){
            return;
        }
...
//            String tagString = pySb.toString().substring(0, 1);
            if (target.matches("[A-Z]")) {//如果是A-Z字母开头
                indexPinyinBean.setBaseIndexTag(target);
                if (isNeedRealIndex) {//如果需要真实的索引数据源
                    if (!mIndexDatas.contains(target)) {//则判断是否已经将这个索引添加进去，若没有则添加
                        mIndexDatas.add(target);
                    }
                }
            } else {//特殊字母这里统一用#处理
